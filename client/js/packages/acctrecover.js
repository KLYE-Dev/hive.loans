...
//Cheers to @reazuliqbal for having this up on https://github.com/CodeBull/HiveAccountRecovery/
...
$(document).ready(async function () {
        ...
        if (username === '') {
            ...
            if (isEligible) {
                ...
                if (activeKey === '') {
                    ...
                    dhiveclient.broadcast.sendOperations([op], dhive.PrivateKey.from(activeKey))
                        .then((r) => {
                            console.log(r);
    ...
    $('#recover-account').submit(async function (e) {
        ...
        if (username === '') {
            ...
            const recoveryRequest = await dhiveclient.database.call('get_recovery_request', [username])
            ...
            if (recoveryRequest && new Date(`${recoveryRequest.expires}Z`).getTime() > new Date().getTime()) {
                ...
                dhiveclient.broadcast.sendOperations([op], [dhive.PrivateKey.from(oldOwner.owner), dhive.PrivateKey.from(newOwner.owner)])
                    .then(async (r) => {
                        console.log(r);
                        ...
                        dhiveclient.broadcast.updateAccount(accountUpdateObj, dhive.PrivateKey.from(newOwner.owner))
                            .then(async (r) => {
                                console.log(r);
                            })
                            .catch(e => {
    ...
    $('#change-recovery-account').submit(async function (e) {
        ...
        if (username !== '' && newRecovery !== '' && password !== '') {
            ...
            dhiveclient.broadcast.sendOperations([op], dhive.PrivateKey.from(ownerKey.owner))
                .then((r) => {
                    console.log(r);
