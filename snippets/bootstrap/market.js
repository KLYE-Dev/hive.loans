...
actions.cancel = async (payload) => {
  ...
  if (api.assert(isSignedWithActiveKey === true, 'you must use a custom_json signed with your active key')
    ...
    let order = null;
    // get order
    if (api.refHiveBlockNumber < '${FORK_BLOCK_NUMBER_THREE}$' && Number.isInteger(id)) {
      order = await api.db.findOne(table, { _id: id });
    } else if (api.refHiveBlockNumber >= '${FORK_BLOCK_NUMBER_THREE}$' && typeof id === 'string' && id.length < 50) {
...
actions.buy = async (payload) => {
  ...
  // buy (quantity) of (symbol) at (price)(HIVE_SYMBOL) per (symbol)
  if (api.assert(isSignedWithActiveKey === true, 'you must use a custom_json signed with your active key')
    && api.assert(
      price && typeof price === 'string' && !api.BigNumber(price).isNaN()
      && symbol && typeof symbol === 'string' && symbol !== HIVE_SYMBOL
    ...
    if (api.assert(token
      ...
      if (api.assert(api.refHiveBlockNumber < '${FORK_BLOCK_NUMBER_TWO}$' || api.BigNumber(nbTokensToLock).gte('0.001'), 'order cannot be placed as it cannot be filled')) {
        // lock HIVE_SYMBOL tokens
        const res = await api.executeSmartContract('tokens', 'transferToContract', { symbol: HIVE_SYMBOL, quantity: nbTokensToLock, to: CONTRACT_NAME });

        if (res.errors === undefined &&
          res.events && res.events.find(el => el.contract === 'tokens' && el.event === 'transferToContract' && el.data.from === api.sender && el.data.to === CONTRACT_NAME && el.data.quantity === nbTokensToLock && el.data.symbol === HIVE_SYMBOL) !== undefined) {
...
actions.sell = async (payload) => {
  const { symbol, quantity, price, expiration, isSignedWithActiveKey } = payload;
  // sell (quantity) of (symbol) at (price)(HIVE_SYMBOL) per (symbol)
  if (api.assert(isSignedWithActiveKey === true, 'you must use a custom_json signed with your active key')
    && api.assert(
      price && typeof price === 'string' && !api.BigNumber(price).isNaN()
      && symbol && typeof symbol === 'string' && symbol !== HIVE_SYMBOL
    ...
    if (api.assert(token
      ...
      if (api.assert(api.refHiveBlockNumber < '${FORK_BLOCK_NUMBER_TWO}$' || api.BigNumber(nbTokensToFillOrder).gte('0.001'), 'order cannot be placed as it cannot be filled')) {
        // initiate a transfer from api.sender to contract balance
        // lock symbol tokens
    ...
    while (inc < nbOrders && api.BigNumber(buyOrder.quantity).gt(0)) {
      ...
      if (api.BigNumber(buyOrder.quantity).lte(sellOrder.quantity)) {
        ...
        if (api.assert(api.BigNumber(qtyTokensToSend).gt(0)
          ...
          res = await api.transferTokens(sellOrder.account, HIVE_SYMBOL, qtyTokensToSend, 'user');

          if (res.errors) {
            api.debug(res.errors)
            api.debug('TXID: ' + api.transactionId)
            api.debug(sellOrder.account)
            api.debug(HIVE_SYMBOL)
          ...
          if (api.BigNumber(qtyLeftSellOrder).gt(0)
            && (api.refHiveBlockNumber < '${FORK_BLOCK_NUMBER_TWO}$' || api.BigNumber(nbTokensToFillOrder).gte('0.001'))) {
          ...
          if (api.BigNumber(tokensToUnlock).gt(0)) {
            await api.transferTokens(account, HIVE_SYMBOL, tokensToUnlock, 'user');
          }
        ...
        if (api.assert(api.BigNumber(qtyTokensToSend).gt(0)
          ...
          if (api.refHiveBlockNumber >= '${FORK_BLOCK_NUMBER_TWO}$' && api.BigNumber(nbTokensToFillOrder).lt('0.001')) {
            await api.transferTokens(account, HIVE_SYMBOL, buyOrder.tokensLocked, 'user');
    ...
    while (inc < nbOrders && api.BigNumber(sellOrder.quantity).gt(0)) {
      ...
      if (api.BigNumber(sellOrder.quantity).lte(buyOrder.quantity)) {
        ...
        if (api.assert(api.BigNumber(qtyTokensToSend).gt(0)
          ...
          // transfer the tokens to the seller
          res = await api.transferTokens(account, HIVE_SYMBOL, qtyTokensToSend, 'user');
          ...
          if (api.BigNumber(qtyLeftBuyOrder).gt(0)
            ...
            if (api.BigNumber(buyOrdertokensLocked).gt(0)) {
              await api.transferTokens(buyOrder.account, HIVE_SYMBOL, buyOrdertokensLocked, 'user');
            }
            await api.db.remove('buyBook', buyOrder);
          }
...
const updatePriceMetrics = async (symbol, price) => {
  ...
  if (metric.lastDayPriceExpiration < timestampSec) {
    ...
    metric.priceChangeHive = api.BigNumber(price).minus(metric.lastDayPrice).toFixed(3);
